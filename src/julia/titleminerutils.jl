...
"""
   `top_k_dict_words(d::Dict{String,Int64}, k::Int64)::Tuple{Vector{String},Vector{Int64}}`

Return two vectors with the top-k words and with their frequencies, respectively, among the words included in the dictionary with keys the words and values their frequencies.
"""
function top_k_dict_words(d::Dict{String,Int64}, k::Int64)::Tuple{Vector{String},Vector{Int64}}
...
"""
   `conf_word_frequencies(conf_name::String, k::Int64)::Tuple{Vector{String},Vector{Int64}}`

Return two vectors with the top-k words and with their frequencies, respectively, among the words which appear in the titles of all the conference papers. If `k` is zero, then the vectors includes all the words and all their frequencies.
"""
function conf_word_frequencies(conf_name::String, k::Int64)::Tuple{Vector{String},Vector{Int64}}
    ...
    if (k > 0)
        ...
        return top_k_dict_words(w_dict, length(keys(w_dict)))
    end
end

"""
   `area_word_frequencies(conf_name::String, k::Int64)::Tuple{Vector{String},Vector{Int64}}`

Return two vectors with the top-k words and with their frequencies, respectively, among the words which appear in the titles of all papers included in the area.
"""
...
"""
   `word_evolution(conf_name::String, step::Int64, k::Int64)::Tuple{Vector{Int64},Dict{String,Vector{Int64}}}`

Return a vector with the number of words in all the titles of the papers in each interval of length `step` and a dictionary which, for any word among the top-k, has as value the vector of its occurrences in all the titles of the papers in each interval of length `step`.
"""
function word_evolution(conf_name::String, step::Int64, k::Int64)::Tuple{Vector{Int64},Dict{String,Vector{Int64}}}
    first_year::Int64, last_year::Int64 = first_last_year(conf_name)
    words::Vector{String}, _ = conf_word_frequencies(conf_name, k)
    sort!(words)
