...
function data_mining_html(io::IOStream, conf_name::String, co_author_step::Int64, conf_array::Vector{String})
    ...
    coauthorship_plot(conf_name, co_author_step, "/co_authorship_period")
    ...
    similarity_indices_plot(filter(c -> c != conf_name, conf_array), [conf_name], "/similarity_values")
    conf_index = findfirst(x -> x == conf_name, conf_array)
    if (conf_index === nothing)
        new_author_mean_bar_chart([[conf_name]; conf_array], 1, "/new_author_perc_bar")
    else
        new_author_mean_bar_chart(conf_array, conf_index, "/new_author_perc_bar")
    end
...
function data_mining_html(io::IOStream, conf::Array{String}, html_dir::String, co_author_step::Int64)
    paper_year_plot(conf, 1, "/paper_year")
    cp(path_to_files * "images/" * conf[1] * "/paper_year.html", path_to_files * "html/" * html_dir * "/paper_year.html", force=true)
    author_year_plot(conf, 1, "/author_year")
    cp(path_to_files * "images/" * conf[1] * "/author_year.html", path_to_files * "html/" * html_dir * "/author_year.html", force=true)
    perc_new_author_year_plot(conf, 1, "/perc_new_author_year")
    cp(path_to_files * "images/" * conf[1] * "/perc_new_author_year.html", path_to_files * "html/" * html_dir * "/perc_new_author_year.html", force=true)
    new_author_mean_bar_chart(conf, 0, "/new_author_perc_bar")
    cp(path_to_files * "images/" * conf[1] * "/new_author_perc_bar.html", path_to_files * "html/" * html_dir * "/new_author_perc_bar.html", force=true)
    coauthorship_perc_plot(conf, 1, "/co_authorship_perc")
    cp(path_to_files * "images/" * conf[1] * "/co_authorship_perc.html", path_to_files * "html/" * html_dir * "/co_authorship_perc.html", force=true)
    similarity_indices_plot(conf, conf, "/similarity_values")
    cp(path_to_files * "images/" * conf[1] * "/similarity_values.html", path_to_files * "html/" * html_dir * "/similarity_values.html", force=true)
    # 
    write(io, "<a id=\"data\"></a><h2>Data mining</h2>\n<h3>Evolution of paper and author numbers</h3>\n<p>The following picture shows, for each conference, the evolution of the number of its papers per year.</p>\n<iframe title=\"Evolution of paper number\" src=\"paper_year.html\" width=\"600\" height=\"600\"></iframe>\n<p>The following picture shows, for each conference, the evolution of the number of its authors per year.</p>\n<iframe title=\"Evolution of author number\" src=\"author_year.html\" width=\"600\" height=\"600\"></iframe>\n<p>The following picture shows, for each conference, the evolution of the percentage of its <i>new</i> distinct authors per year.</p>\n<iframe title=\"Evolution of percentage of new authors\" src=\"perc_new_author_year.html\" width=\"600\" height=\"600\"></iframe>\n<p>The following picture shows, for each conference, the <i>average</i> percentage of new authors at each year.</p>\n<iframe title=\"Comparison of average percentage of new authors\" src=\"new_author_perc_bar.html\" width=\"600\" height=\"600\"></iframe>\n<p>The following picture shows, for each conference, the percentage of papers with the specified number of co-authors (in particular, for each number k of co-authors, the plot shows the percentage of papers, over all papers, with k authors).</p>\n<iframe title=\"Percentage of papers with specificic co-authorship size\" src=\"co_authorship_perc.html\" width=\"600\" height=\"600\"></iframe>\n<h3>Similarity between conferences</h3>\n<p>Given two sets A and B, the Jaccard index J(A,B) is equal to the ratio between the cardinality of their intersection and the cardinality of their union. The Sorensen-Dice index of similarity is equal to 2J(A,B)/(1+J(A,B)). The following picture shows the Sorensen-Dice index of similarity computed by comparing, for each pair of conferences C1 and C2, the set of C1's authors with the set of C2's authors.</p>\n<iframe title=\"Sorensen-Dice index of similarity\" src=\"similarity_values.html\" width=\"600\" height=\"600\"></iframe>\n")
end
...
function sex_analysis_html(io::IOStream, conf::Array{String}, html_dir::String)
    missing_sex_bar_chart(conf, 0, "no_sex_author_perc")
    cp(path_to_files * "images/" * conf[1] * "/no_sex_author_perc.html", path_to_files * "html/" * html_dir * "/no_sex_author_perc.html", force=true)
    female_male_ratio_plot(conf, 1, "female_male_ratio")
    cp(path_to_files * "images/" * conf[1] * "/female_male_ratio.html", path_to_files * "html/" * html_dir * "/female_male_ratio.html", force=true)
    # 
...
function graph_mining_html(io::IOStream, conf::Array{String}, html_dir::String)
    nn_ne_d_lcc = statistics(conf)
    densification_plot(conf, 1, "/densification")
    cp(path_to_files * "images/" * conf[1] * "/densification.html", path_to_files * "html/" * html_dir * "/densification.html", force=true)
    diameter_plot(conf, 1, "/diameter")
    cp(path_to_files * "images/" * conf[1] * "/diameter.html", path_to_files * "html/" * html_dir * "/diameter.html", force=true)
    degree_separation_plot(conf, 1, "/degrees_separation")
    cp(path_to_files * "images/" * conf[1] * "/degrees_separation.html", path_to_files * "html/" * html_dir * "/degrees_separation.html", force=true)
    # 
    ...
    for c in 1:length(conf)
        nn = string(nn_ne_d_lcc[c][1])
        ne = string(nn_ne_d_lcc[c][2])
        d = string(Base._round_invstep(nn_ne_d_lcc[c][3], 1 / 0.0001, RoundNearest))
        lcc = string(trunc(Int64, 100 * Base._round_invstep(nn_ne_d_lcc[c][4], 1 / 0.01, RoundNearest)))
...
function temporal_graph_html(io::IOStream, conf::Array{String})
    ...
    for c in 1:length(conf)
        top::Vector{String} = top_k_closeness(conf[c], 3, false)[2]
        tkn[c, 1] = top[1]
        tkn[c, 2] = top[2]
        tkn[c, 3] = top[3]
    end
...
"""
   `conf_web_page(conf::Array{String}, web_page::BitVector, html_dir::String, title::String, sub_title::String, co_author_step::Int64)`

Generate a comparative analysis web page of the conferences in `conf` (by using `title` and `sub_title` for the title and the sub-title of the page): the web page is saved in the directory `html_dir`. The vector `web_page` of Boolean values specifies whether the web page of each conference is available. The value of `co_author_step` is used for the computation of the co-authorship size distribution.
"""
function conf_web_page(conf::Array{String}, web_page::BitVector, html_dir::String, title::String, sub_title::String, co_author_step::Int64)
    mkpath(path_to_files * "html/" * html_dir)
    cp(path_to_files * "html/cmstyle.css", path_to_files * "html/" * html_dir * "/cmstyle.css", force=true)
    cp(path_to_files * "html/pc.jpg", path_to_files * "html/" * html_dir * "/pc.jpg", force=true)
    io::IOStream = open(path_to_files * "html/" * html_dir * "/index.html", "w")
    start_html(io, conf, web_page, title, sub_title)
    data_mining_html(io, conf, html_dir, co_author_step)
